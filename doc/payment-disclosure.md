...
Frequent users of shielded transactions, payment processors, exchanges, block explorers

### Experimental Feature

This is an experimental feature.  Enable it by launching `hushd` with flags:

    hushd -experimentalfeatures -paymentdisclosure -debug=paymentdisclosure -txindex=1

These flags can also be set as options in `hush.conf`.

All nodes that generate or validate payment disclosures must run with `txindex=1` enabled.

### Background

Payment Disclosure is an implementation of the work-in-progress Payment Disclosure upstream ZIP [1].
...
[1] https://github.com/hush/zips/pull/119
...
joinsplit output index maps to a given payment they made.  It is possible to
construct this from `debug.log`.  Ideas and feedback are most welcome on how to
